...
def rec_search(rec_type_num):
    ...
    with open(f'{rec_type[rec_type_num]}.csv', newline='') as rec_f:
        try:
            found_rec = csv.reader(rec_f)[list(map(int,[row[0] for row in list(csv.reader(rec_f))])).index(rec_id)]
            print('Record found\n'+'-'*20+'\n'+f"Name: {found_rec[1]}\n{person_type_keywords[rec_type[rec_type_num]]['title'].capitalize()}: {found_rec[2]}\n{person_type_keywords[rec_type[rec_type_num]]['date']}: {found_rec[3]}")
        except ValueError:
...
def rec_remove(rec_type_num):
    ...
    with open(f'{rec_type[rec_type_num]}.csv', newline='') as rec_f:
        ...
        recs.remove(recs[list(map(int,[row[0] for row in list(csv.reader(rec_f))])).index(rec_id)])
